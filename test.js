async function main() {
    ...
    function addSupplementaryGrade()
    {
        ...
        data.forEach(item => {
            // Find the exact match or the next lower grade
            let lowerGrade = supplementaryGrade.find(grade => item.score >= grade.score);
            if (!lowerGrade) {
                // If score is below the lowest grade, assign the lowest grade
                lowerGrade = supplementaryGrade[supplementaryGrade.length - 1];
            }
    
            // Assuming scores are sorted in descending order in supplementaryGrade
            const lowerGradeIndex = supplementaryGrade.indexOf(lowerGrade);
            const upperGrade = supplementaryGrade[lowerGradeIndex - 1]; // Get the next grade up
    
            if (upperGrade) {
                // Interpolate only if there's a valid upper grade
                const scoreDiff = upperGrade.score - lowerGrade.score;
                const ratio = (item.score - lowerGrade.score) / scoreDiff;
                const interpolatedFourRounding = lowerGrade.fourRounding + ratio * (upperGrade.fourRounding - lowerGrade.fourRounding);
    
                // Assuming toFixed is a custom function or using Number's toFixed method for rounding
                item.letter = lowerGrade.letter; // This could be adjusted if letter assignment needs interpolation
                item.fourRounding = parseFloat(interpolatedFourRounding.toFixed(2)); // Adjust precision as needed
            } else {
                // Handle cases where score matches the highest grade or above
                item.letter = lowerGrade.letter;
                item.fourRounding = lowerGrade.fourRounding;
            }
        });
